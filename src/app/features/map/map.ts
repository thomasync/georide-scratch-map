...
export class Map {
	...
	private computeStatsData(): StatsModalData {
		return buildStatsData({
			tripsWithCoords: this.tripsWithCoords,
			allTripsWithCoords: this.allTripsWithCoords,
			departments: this.departments,
			cellsByResolution: this.cellsByResolution,
			deptResolution: this.mapSettings.deptResolution() as H3Resolution,
			enrichDepartments: (d, c, r, idx) => this.h3.enrichDepartmentsWithCoverage(d, c, r, idx),
			fuelPrices: this.fuelPrices(),
			fuelType: this.fuelType,
			allR7Data: this.allR7Data,
		});
	}

	private tripSeason(year: number, month: number): string {
		return tripSeason(year, month);
	}

	private seasonSortKey(label: string): number {
		return seasonSortKey(label);
	}

	private countryForCoords(lat: number, lon: number): string {
		return countryForCoords(lat, lon);
	}

	private findDeptCodeForPoint(lng: number, lat: number): string | null {
		return findDeptCodeForPoint(this.departments, lng, lat);
	}
	...
	private buildRecapData(): void {
		...
		// On s'appuie sur savedNewCellsR7 (l'ensemble canonique) et non sur newCellsR7,
		// qui est vidé de façon synchrone quand le récap a déjà été dismissé alors que
		// ce calcul est différé : sinon le récap (et donc le bouton) afficherait 0 lieu.
		if (this.savedNewCellsR7.size === 0) {
			this.newCellsRecapData.set(null);
			return;
		}
		// Collect only dates where new cells were discovered
		const newCellDates = new Set<string>();
		for (const cell of this.savedNewCellsR7) {
		...
		this.newCellsRecapData.set({
			newHexCount: this.savedNewCellsR7.size,
			trips,
	...
	private computeNewCellsDeptStats(): NewCellsRecapData['depts'] {
		...
		for (const feature of this.departments.features) {
			...
			const newCount = cells.filter((c) => this.savedNewCellsR7.has(c)).length;
	...
	/**
	 * Met en surbrillance les cellules H3 découvertes durant le mois `monthKey` (format AAAA-MM)
	 * et zoome sur la zone — même rendu que le bouton « Voir les trajets » du récap nouveaux lieux.
	 */
	...
	private viewTripsForNewCells(): void {
		if (!this.allR7Data) return;
		const newCellDates = new Set<string>();
		...
		this.logger.log(
			'Map',
			`[viewTripsForNewCells] indices=${indices.size} dates=${[...newCellDates].join(',')} zoom=${this.map?.getZoom().toFixed(2)} deptThreshold=${this.deptThreshold}`,
		);
