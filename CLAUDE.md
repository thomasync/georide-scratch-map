...
GeoRide motorcycle trip "scratch" map (Angular 22, zoneless, MapLibre, H3). Tests: Vitest (unit) + Playwright (e2e).
...
- Prettier: tabs, `printWidth` 120, single quotes (runs automatically on commit).
- Components: signals (`input()`/`output()`/`viewChild()`), `inject()`, `@if`/`@for` control flow.
